...
app.use((req, res, next) => {
  if (req.originalUrl === "/api/subscriptions/webhook" || req.originalUrl === "/api/webhook") {
    next();
  } else {
...
app.post("/api/webhook", express.raw({ type: "application/json" }), SubscriptionController.stripeWebhook);
